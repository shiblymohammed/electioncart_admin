    ...
    <div className="flex items-center gap-3">
      ...
      <button
        ...
        className={`
          ...
          ${isPopular ? 'bg-warning shadow-glow-warning' : 'bg-dark-border'}
          ${disabled || loading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:opacity-90'}
        `}
        role="switch"
        aria-checked={isPopular}
        title={isPopular ? `Popular #${popularOrder} - Click to unmark` : disabled ? 'Limit reached (3/3)' : 'Click to mark as popular'}
      >
        {loading ? (
      ...
      <div className="flex items-center gap-2">
        {isPopular ? (
          <>
            <span className="inline-flex items-center gap-1 px-2 py-1 bg-warning/20 text-warning text-xs font-semibold rounded-full border border-warning/30 shadow-glow-warning">
          ...
          <span className="text-xs text-text-muted">
            {disabled ? 'Limit reached (3/3)' : 'Not popular'}
          </span>
        )}
      </div>
    </div>
